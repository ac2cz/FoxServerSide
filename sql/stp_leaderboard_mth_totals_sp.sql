DROP PROCEDURE IF EXISTS StpLeaderboardMthTotals;

DELIMITER //
CREATE PROCEDURE StpLeaderboardMthTotals()
BEGIN
...
select id, receiver, 
sum(case when source like '%duv' then 1 else 0 end) DUV, 
sum(case when source like '%highspeed' then 1 else 0 end) HighSpeed, 
sum(case when source like '%bpsk' then 1 else 0 end) PSK, 
sum(case when timestampdiff(DAY,date_time,now()) < 7 then 1 else 0 end) last 
from STP_HEADER group by receiver order by DUV DESC;
...
set STP_HEADER_COUNT_TEMP.DUV = STP_HEADER_COUNT_TEMP.DUV + STP_HEADER_ARCHIVE_COUNT.duv_total,
    ...
    STP_HEADER_COUNT_TEMP.PSK = STP_HEADER_COUNT_TEMP.PSK + STP_HEADER_ARCHIVE_COUNT.psk_total;
...
as 
select t1.id, t1.receiver, duv_total, highspeed_total, psk_total 
...
(id, receiver, DUV, HighSpeed, PSK)
select id, receiver, duv_total, highspeed_total, psk_total
from STP_HEADER_ARCHIVE_TEMP;
...
sum(PSK) PSK,
sum(DUV) + sum(HighSpeed) +sum(PSK) total,
sum(last) last 
from STP_HEADER_COUNT_TEMP
group by receiver
order by total DESC;
